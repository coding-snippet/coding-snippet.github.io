...
df_basel_pop <- read_excel("data/100128.xlsx")  %>%
  janitor::clean_names() %>% 
  filter(datum == "2023-12-31")
...
labels <- c(
  ...
  "105-109","110-114","115-119"
)

df_basel_pop_1 <- df_basel_pop %>%
  arrange(alter) %>% 
  mutate(
    bin = cut(alter, breaks = bins, labels = labels, right = FALSE),
    bin = fct_inorder(bin)
  ) %>% 
  group_by(name_gender_bfs, bin) %>% 
  summarize(
    count = sum(anzahl)
  ) %>% 
  ungroup() %>% 
...
plot <- df_basel_pop_1 %>% 
  ggplot(
    aes(
      x = count,
      y = bin,
      fill = name_gender_bfs
    )
  ) +
  geom_col() +
  scale_x_continuous(
    breaks = pretty(range(df_basel_pop_1$count), n = 9),
    labels = abs(pretty(range(df_basel_pop_1$count), n = 9))
  ) +
  scale_fill_manual(
    values = c("Mann" = "#1346d4", "Frau" = "#d4133a"),
    breaks = c("Mann", "Frau")
  ) +
  labs(
    title = "Altersbaum der Stadt Basel",
    caption = "Quelle Originaldaten:\nhttps://data.bs.ch/explore/dataset/100128",
    y = "Alter",
    x = "Population"
  ) +
  theme_bw() +
